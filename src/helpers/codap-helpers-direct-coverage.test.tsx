/**
 * This test file directly targets specific lines in codap-helpers.tsx for coverage
 * Lines targeted: 205, 236, 500
 */
...
describe('codap-helpers direct coverage tests', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  // Test for line 205 - addMeasure with existing attribute update
  test('addMeasure should update an existing attribute when measureName is provided', async () => {
    // Mock implementation
    (codapInterface.sendRequest as jest.Mock).mockImplementation((request: any) => {
      ...
      if (request.resource && request.resource.includes('attributeList')) {
        return Promise.resolve({
          success: true,
          values: [
    ...
    const hasUpdateCall = calls.some(call => 
  ...
  test('getNewExperimentInfo should throw an error when data context is not found', async () => {
    // Mock implementation
    (codapInterface.sendRequest as jest.Mock).mockImplementation((request: any) => {
  ...
  test('renameAttribute should handle attributes without formulas', async () => {
    // Mock implementation
    (codapInterface.sendRequest as jest.Mock).mockImplementation((request: any) => {
      if (request.resource && request.resource.includes('collectionList')) {
        return Promise.resolve({
      ...
      if (request.resource && request.resource.includes('allCases')) {
        return Promise.resolve({
          success: true,
          values: {
            cases: [
